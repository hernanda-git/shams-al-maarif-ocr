...
Usage:
  python3 translate_en.py --gentle   # Process 6 pages (2 API calls, 15s apart)
  python3 translate_en.py --range 1-50
...
# PROMPT
# ─────────────────────────────────────────────────────────
SYSTEM_PROMPT = """You are a faithful translator of Arabic occult and esoteric manuscripts into English. Your ONLY task is VERBATIM translation.

## Mandatory Rules
1. DO NOT summarize. 2. DO NOT paraphrase. 3. DO NOT simplify. 4. DO NOT modernize. 5. DO NOT reinterpret.
6. DO NOT explain inside the translation. 7. DO NOT rewrite sentence structure unless English grammar requires it.
8. DO NOT omit repeated phrases, honorifics, invocations, technical terminology, rhetorical emphasis, or ambiguities.

## Technical Terms
If no exact English equivalent exists, retain Arabic transliteration.
Examples: رمل→Raml, شكل→Shakl, وتد→Watad, طالع→Tali', روحانية→Ruhaniyyah

## Grid/Diagram Content
Translate surrounding explanatory text. Preserve all Arabic letters, numbers, and grid structures as-is.
Mark as [Grid content preserved as-is] and include the original.
...
## Final Rule
Accuracy over readability. Faithfulness over elegance. Verbatim preservation over stylistic fluency."""
...
def translate_batch(page_files):
    ...
    combined = "\n\n".join(parts)
    user_msg = (
        f"Translate each page below verbatim from Arabic to English. "
        f"IMPORTANT: Begin each page's output with its page marker — for example, "
        f"--- PAGE 30 TRANSLATION --- then the Arabic: and English: sections. "
        f"Do NOT combine pages. Output SEPARATE sections for each page.\n\n"
        f"{combined}"
    )
...
def _split_and_write(page_files, output_text):
    ...
    # Strategy 1: "--- PAGE N TRANSLATION ---" markers
    # Strategy 2: "--- PAGE N ---" markers (input format echoed back)
    for pattern in [
        r'--- PAGE (\d+) TRANSLATION ---\n?(.*?)(?=--- PAGE \d+ TRANSLATION ---|\Z)',
        r'--- PAGE (\d+) ---\n?(.*?)(?=--- PAGE \d+ ---|\Z)'
    ]:
    ...
    for pf in page_files:
        ...
        if pn in by_page and by_page[pn]:
            with open(outpath, 'w', encoding='utf-8') as f:
                f.write(by_page[pn])
            results.append((pf, by_page[pn], None))
        else:
            with open(outpath, 'w', encoding='utf-8') as f:
                f.write(output_text if len(page_files) == 1
                        else f"Arabic:\n[combined output — see batch]\n\nEnglish:\n{output_text}")
            results.append((pf, output_text, None))
...
def process_next_batch(pages_per_run=None, api_delay=None):
...
def show_status():
    ...
    print(f"  Output files: {out_count} in enriched_en/")
    print(f"{'='*56}")
...
def _translate_single(text):
    ...
    payload_template = {
        ...
        "input": [
            {"role": "developer", "content": SYSTEM_PROMPT},
            {"role": "user", "content": f"Translate this Arabic page verbatim into English (Arabic: then English:):\n\n{text}"}
        ],
        "max_output_tokens": 8192,
        "temperature": 0.1
    }
...
def main():
    ...
    if args.status:
        ...
        if m:
            ...
            pending = get_pending_pages()
            in_range = [pf for pf in pending if start <= get_page_num(pf) <= end]
            if not in_range:
                print(f"All pages in range {start}-{end} already done.")
                return
            process_next_batch(pages_per_run=len(in_range), api_delay=API_DELAY)
        else:
